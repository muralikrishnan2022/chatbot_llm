...
def ingest_file(file_path, session_vector_store):
    """Ingest a PDF or image file into the session-specific vector store."""
    ...
    try:
        ...
        logger.info(f"Adding {len(split_documents)} documents to the session vector store")
        session_vector_store.add_documents(documents=split_documents, ids=uuids)
...
def main():
    ...
    from config import initialize_models, initialize_vector_stores, setup_signal_handlers
    setup_signal_handlers()
    models = initialize_models()
    default_vector_store, _, _ = initialize_vector_stores(models.embeddings_ollama)
